...
export default function DoctorAppointmentsPage() {
  ...
  const pendingTotal = totalAppts.filter(a => a.status === 'scheduled' || a.status === 'waiting' || a.status === 'checked-in').length;
  const activeAppt = totalAppts.find(a => a.status === 'in_consultation');
            ...
            <Table>
              ...
              <TableBody>
                {sortedAppointments.map((appt) => {
                  const patient = patients?.find(p => p.id === appt.patient_id);
                  const isConsulting = appt.status === 'in_consultation';
                    ...
                    <TableRow key={appt.id} className={`border-b border-slate-100 dark:border-slate-800/50 transition-colors ${isConsulting ? 'bg-blue-50/50 dark:bg-blue-900/10' : 'hover:bg-slate-50/50 dark:hover:bg-slate-900/50'}`}>
                      ...
                      <TableCell>
                        <Badge variant={
                          ...
                          appt.status === 'waiting' || appt.status === 'checked-in' ? 'secondary' : 
                          appt.status === 'in_consultation' ? 'default' : 'default'
                        } className={`capitalize px-3 py-1 text-xs font-semibold rounded-full ${appt.status === 'in_consultation' ? 'bg-blue-500' : ''}`}>
...
with open("frontend/src/app/doctor/appointments/page.tsx", "w") as f:
    f.write(content)

print("Redesigned doctor appointments")
